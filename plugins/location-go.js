...
const locations = {
  ...
  puncak: {
    ...
    message: 'Selamat datang di Puncak Perkasa! Kekuatanmu meningkat 100 kali lipat. 💪🌟',
    multi: 100,
  },
  benteng: {
    name: 'benteng',
    ...
    message: 'Selamat datang di Benteng Tangguh! Pertahananmu meningkat 100 kali lipat. 🛡️🌟',
    multi: 100,
  },
  awan: {
    name: 'awan',
    ...
    message: 'Selamat datang di Sumber Psikis! Kemampuan psikismu meningkat 100 kali lipat. 🔮🌟',
    multi: 100,
  },
  kilat: {
    name: 'kilat',
    ...
    message: 'Selamat datang di Lintasan Kilat! Kecepatanmu meningkat 100 kali lipat. 🏃💨🌟',
    multi: 100,
  },
};
...
const handler = async (m, { usedPrefix, args }) => {
  ...
  if (args.length !== 1) {
    return m.reply(`
Gunakan format: ${usedPrefix}go [tempat]
...
`.trim());
  ...
  if (requirementsNotMet.length > 0) {
    ...
    return m.reply(errorMessage + Object.entries(requirement).map(([reqStat, reqValue]) => `${global.rpg.emoticon(reqStat)} ${reqStat} ${toSimple(reqValue)}`).join('\n'));
  }
