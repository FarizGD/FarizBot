    ...
    try {
        ...
        for (let name in plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (name != 'plugins/admin-unbanchat.js' && name != 'plugins/owner-kick.js') {
                    let chat = await db.chats.get(m.chat)
                    if (chat.banned) return // Except this
                }
                if ((name != 'plugins/owner-unbanuser.js' && name != 'plugins/ban-checktime.js') && _user?.banned)
                    return
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: 'Perintah ini hanya dapat digunakan oleh _*Owner Utama*_!',
